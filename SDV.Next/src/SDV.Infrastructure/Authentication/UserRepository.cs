    ...
    public async Task<User?> FindByUsernameAsync(string username, CancellationToken cancellationToken)
    {
        const string sql = """
            SELECT
                ...
                CAST(u.Usu_estado AS UNSIGNED) AS IsActive,
                p.Priv_Id AS RoleId,
                ...
                CAST(u.Usu_act_contrasena AS UNSIGNED) AS MustChangePassword
            FROM tbl_usuario AS u
            INNER JOIN tbl_privilegio AS p ON u.Usu_Priv_Id = p.Priv_Id
            INNER JOIN tbl_rutas AS r ON u.Usu_Ru_Id = r.Ru_Id
            INNER JOIN tbl_distrito AS d ON r.Ru_Dist_Id = d.Dist_Id
            INNER JOIN tbl_canal AS c ON d.Dist_Ca_Id = c.Ca_Id
            INNER JOIN tbl_distribuidora AS dis ON c.Ca_Dis_Id = dis.Dis_Id
            INNER JOIN tbl_division AS di ON dis.Dis_Di_Id = di.Di_Id
            INNER JOIN tbl_pais AS pa ON di.Di_P_Id = pa.P_Id
